...
⚠️ 副業・お仕事系の発信では「必ず月◯万円稼げる」など収入を保証する表現は景品表示法上リスクがあるため、生成ルールで禁止しています。実績・お客様の声も実際にあるものだけを記入してください。`ANTHROPIC_API_KEY` があれば完成形の文章、なければ(…)を埋めるだけで使える穴埋め式テンプレートになります。
