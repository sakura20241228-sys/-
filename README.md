...
さらに「書き方の条件」(`src/copywriter.py` の `ALGORITHM_RULES`)として、以下も全投稿に適用されます。

- 最初の1行で続きを読ませる(好奇心・驚き・危機感のどれかを入れる)
- 本文は300文字前後。1文を短く、改行を多く
- 難しい言葉・AIっぽい表現(「いかがでしたか」等)は禁止
- 最後は保存したくなる締めにする
- 読者が「自分のことだ」と思える内容にする

各案には投稿文に加えて **タイトル候補3つ**(Threadsはタイトル欄がないため、1行目の差し替え候補)と **この投稿の狙い(なぜ伸びやすいか)** の解説が付きます。
...
⚠️ 副業・お仕事系の発信では、**「稼げる」「稼ぐ」という言葉自体**と、収入を保証する表現(「必ず月◯万円」等)を生成ルールで禁止しています(「収入をつくる」「おうちでお仕事」などに言い換え)。禁止ワードは `src/config.py` の `NG_WORDS` で管理していて、下書きにNGワードが混ざった場合は ⚠️ 警告が表示されます。実績・お客様の声も実際にあるものだけを記入してください。
