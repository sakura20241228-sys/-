...
# ===== 書き方の条件(アルゴリズム)。すべての投稿文に適用 =====
ALGORITHM_RULES = textwrap.dedent("""\
    - 最初の1行で続きを読ませる(好奇心・驚き・危機感のどれかを入れる)
    - 本文は300文字前後
    - 1文を短く。改行を多くする
    - 難しい言葉は使わない(小学生でも分かる言葉で)
    - AIっぽい表現は禁止(「〜ではないでしょうか」「いかがでしたか」「〜しましょう」「〜が重要です」「〜がおすすめです」など)
    - 最後は保存したくなる締めにする
    - 読者が「自分のことだ」と思える内容にする""")
...
APPEALS: dict[str, dict] = {
    "共感": {
        ...
        "狙い": "読者が「自分のことだ」と感じて、いいね・返信しやすい。売り込みゼロなので警戒されず、フォローにつながる",
        "書き方": (
            "商品・サービスの宣伝は一切入れない。"
    ...
    "お役立ち": {
        ...
        "狙い": "「あとで見返したい」で保存されやすい。保存・リポストが伸びるとおすすめに載りやすくなる",
        "書き方": (
    ...
    "体験談": {
        ...
        "狙い": "ビフォー→アフターの変化はストーリーとして最後まで読まれやすく、読者が未来を自分ごと化できる",
        "書き方": (
    ...
    "お客様の声": {
        ...
        "狙い": "第三者の声は自分で言うより信頼される。検討中の読者が「私もできるかも」と背中を押される",
        "書き方": (
            "BUSINESS_PROFILEの実績・お客様の声を1つ、会話調で紹介する。"
            "自慢に見えないよう「うれしかった報告」という温度感で書き、"
            "「〜するだけで未来が変わる」手軽さを添えて、"
            "最後は返信したくなる問いかけで締める(DM誘導・リンク誘導はしない)。"
        ),
        "テンプレート": textwrap.dedent("""\
            {hook}
            うれしい報告をいただきました🥹
            ...
            同じことで悩んでるママ、いますか?よかったらコメントで教えてね😌
            #子育てママ"""),
    },
    "告知": {
        "役割": "サービスの案内・行動喚起。共感やお役立ちで信頼を貯めたあとに使う",
        "狙い": "共感・お役立ちで信頼が貯まった読者に出口を見せる投稿。頻度を絞ることで売り込み感を避ける",
        "書き方": (
            "1行目のギャップのあと、ノウハウの説明ではなく"
            "「読者がキュンとする未来」のシーンを具体的に1つ見せる。"
            "「〜するだけ♡」で始められる手軽さを添えて、サービス名を伝える。"
            "締めは「詳しくはプロフィールへ」の軽い誘導まで(DM誘導はしない)。"
            "誇大表現や過度な限定・煽りは使わない。"
        ),
        "テンプレート": textwrap.dedent("""\
            {hook}
...
def _system_prompt() -> str:
    ...
    return textwrap.dedent(f"""\
        ...
        - 「稼げる」「稼ぐ」「稼ぎ」という言葉は一切使わない。
          言い換え例: 「収入をつくる」「おうちでお仕事」「お仕事につながる」「働き方を選べる」
        - 収入額や成果を保証する表現(「必ず月◯万円」「誰でも簡単に」等)も使わない
        - 「DMください」「DMで相談」「DMで送ります」などDMへ誘導する表現は絶対に使わない。
          行動を促すときは「返信(リプ)したくなる問いかけで締める」か
          「詳しくはプロフィールへ」までの軽い誘導にとどめる(成約は投稿ではなく、返信をくれた人とのやり取りの中でつくる)
...
def _generate_with_claude(theme: str, appeal: str, count: int) -> list[str] | None:
    ...
    prompt = textwrap.dedent(f"""\
        ...
        各案は「---」だけの行で区切り、次の形式で出力してください(前置き・説明は不要)。
    ...
    # タイトル候補・狙いの解説つきブロックなので、投稿文500文字ではなくブロック全体で余裕を持って制限
    drafts = [d.lstrip("-").strip()[:1500] for d in drafts]
    return drafts[:count] or None


def _template_draft(theme: str, appeal: str) -> str:
    """APIキーなしでも使える穴埋め式の下書き(タイトル候補・狙いつき)。"""
    profile = config.BUSINESS_PROFILE
    hooks = _hook_candidates(theme)
    hook = random.choice(hooks)
    body = APPEALS[appeal]["テンプレート"].format(
        theme=theme,
        hook=hook,
    ...
    others = [h for h in hooks if h != hook] or hooks
    titles = random.sample(others, k=min(3, len(others)))
...
def ng_words_in(text: str) -> list[str]:
    """投稿文に含まれるNGワード(config.NG_WORDS)を返す。なければ空リスト。"""
    return [w for w in config.NG_WORDS if w in text]
